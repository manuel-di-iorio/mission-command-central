...
import { X, Target, AlertTriangle, Users, MapPin, Shield, ScrollText, AlertOctagon } from 'lucide-react';
    ...
    <div className="p-4 space-y-5">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className={cn(
              "text-[8px] font-bold uppercase px-1.5 py-0.5 rounded border inline-block mt-1",
              classificationColors[mission.classification]
            )}>
              {mission.classification.replace('-', ' ')}
            </div>
          </div>
        </div>
      ...
      <div className="space-y-4">
        <div className="panel-border rounded-lg p-3 bg-secondary/10">
          <p className="text-[10px] uppercase font-bold tracking-widest text-muted-foreground mb-1.5 flex items-center gap-1">
            <ScrollText className="h-3 w-3" /> Objective
          </p>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="panel-border rounded-lg p-2.5 bg-card/50">
            ...
            <p className="text-xs font-bold uppercase tracking-wide flex items-center gap-1.5">
              <div className={cn("h-1.5 w-1.5 rounded-full", mission.priority === 'critical' ? 'bg-destructive' : 'bg-primary')} />
              {mission.priority}
            </p>
          </div>
          <div className="panel-border rounded-lg p-2.5 bg-card/50">
            <p className="text-[9px] uppercase font-bold text-muted-foreground mb-1 tracking-tighter">Current Status</p>
        ...
        <div className="panel-border rounded-lg p-3 space-y-2 border-primary/10">
          <p className="text-[10px] uppercase font-bold tracking-widest text-muted-foreground flex items-center gap-1.5">
            <Shield className="h-3 w-3 text-primary" /> Rules of Engagement
          </p>
        ...
        <div className={`rounded-lg p-3 border ${mission.riskScore >= 70 ? 'border-destructive/30' : 'border-border/50'} ${riskBg}`}>
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              <AlertOctagon className={`h-4 w-4 ${riskColor}`} />
              <span className="text-[10px] uppercase font-bold tracking-widest text-muted-foreground">Threat Vector Analysis</span>
            </div>
            <div className="flex items-baseline gap-0.5">
              <span className={`text-xl font-bold font-mono-data ${riskColor}`}>{mission.riskScore}</span>
        ...
        <div>
          ...
          <div className="space-y-1.5">
            {assignedAssets.length === 0 ? (
              <p className="text-[10px] text-muted-foreground italic bg-secondary/10 p-2 rounded text-center border border-dashed border-border/50">Await asset deployment</p>
            ) : (
              assignedAssets.map((a) => (
                <div key={a.id} className="panel-border rounded-lg p-2 text-[10px] font-bold uppercase bg-card/60 flex items-center justify-between border-primary/5">
                  <div className="flex items-center gap-2">
                    <div className={cn("h-1.5 w-1.5 rounded-full", a.status === 'active' ? 'bg-success' : 'bg-warning')} />
                    {a.name}
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-muted-foreground font-mono text-[8px]">{Math.round(a.telemetry.signalStrength)}% SIG</span>
                    <span className={riskColor}>ACTV</span>
                  </div>
                </div>
              ))
            )}
        ...
        <div className="panel-border rounded-lg p-3">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-1.5">
              ...
              <p className="text-[10px] uppercase font-bold tracking-widest text-muted-foreground">Ops Theatre Geolocation</p>
            </div>
          </div>
        ...
        <div className="pt-2">
          {mission.status === 'draft' ? (
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <button
                onClick={() => updateMission(mission.id, { status: 'aborted' })}
